...
// `run_scan`/`PassStats` and the constants/helpers exclusively used
// inside it moved to `super::wspr_scan` so a production app can call
// the same tested decode path over its own caller-owned baseband, not
// just this bench. Re-imported here rather than referenced via
// `super::wspr_scan::X` throughout, so the rest of this file (arms,
// `scan_body`, the bandwidth experiments) is textually unchanged.
// `instrument`/`fano_hist`/`TONE_SPACING_HZ` above are still imported
// directly — this file's own bandwidth-arm code
// (`tone_amplitudes_inline`, `arm`'s counters) uses them too, not
// just `run_scan`.
use super::wspr_scan::{load_baseband, now_us, run_scan, PassStats, MAX_CANDIDATES, NBB};
...
// this file — nothing here references them anymore.
...
fn pipeline(bin: &[u8], idat: &mut [f32], qdat: &mut [f32], on_decodes: &dyn Fn(&[WsprResult])) {
...
fn probe_alloc_placement() {
    ...
    for extra in [0usize, 1] {
        ...
        log::info!(
            "  ps replica, {extra} extra 4K alloc(s): {p:#010x} (mod 32 = {})",
            p % 32
        );
    }
...
fn scan_body(bin: &'static [u8]) {
    ...
    let on_decodes: &dyn Fn(&[WsprResult]) = match unsafe { core::ptr::addr_of!(ON_DECODES).read() }
    {
        Some(f) => f,
        None => &|_: &[WsprResult]| {},
    };
