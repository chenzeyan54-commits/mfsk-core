...
- **応答の締切は捕捉終了から約 1 025 ms。** 境界の 725 でも、現在
  `ft4_rx::TX_TURNAROUND_BUDGET_MS` が持つ 1 225 でもない。後者だと音声は
  8.0 s に出てフレームは約 200 ms 遅れる — 相手側で DT ≈ +0.2、±1.0 s の
  探索窓の内側ではあるが、相手が自分の時計誤差に必要な余裕を削る。
