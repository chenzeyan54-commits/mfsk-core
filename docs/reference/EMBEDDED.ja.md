...
- **key-up に固定した stage 3 の打ち切り**（`DecodeConfig::key_up_guard_ms`、
  ...
  スロット受信前から `slot_q` を peek して得る。ガードは当初 320 ms だった（締切は取得を
  止めるだけで、BP 中の候補は最後まで走り、それが最大 313 ms 続いたため）。
  **現在は 0**。この余裕が買っていたものは制約ではなかった — 0.5 s が実際に
  何に使われるか、およびこの打ち切りが表現していないもう一つの締切について
  は[送信周期の定義（WSJT-X 本家）](#送信周期の定義wsjt-x-本家)を参照。
...
した `mfsk-core/tests/ft8_embedded_pipeline_mirror.rs`。

**2026-09-20 時点で fine sync は3ボードとも再び無効。** 上の 10 局/スロット
は fine sync 有効かつ締切なしでの計測値であり、実電波では約 292 ms が early
パスの先頭から削られるだけで見返りがない（`cut` が 0 → 3, 15, 10, 11 に増え、
3 スロットが key-up を超過）。`MFSK_FT8_FINE_SYNC` は選択肢として残してある
— 両側とも計測済みで、答えは予算次第で反転するため。
`MFSK_FT8_KEY_UP_GUARD_MS` も同様に全ボードで 0。

### 送信周期の定義（WSJT-X 本家）

上の key-up 打ち切りが前提にしている数字は、すべて WSJT-X のソースに
定数として入っている。FT8 で、T/R 周期の境界を t = 0 とすると:

```text
 0.000 s  境界。ここで送信ウィンドウが開く —
          m_bTxTime = (t2p >= tx1) && (t2p < tx2)、tx1 = 0
          (widgets/mainwindow.cpp:4552, 4596)。同じパスで送信メッセージ
          が auto-sequencer から取られ (txMsg = ui->txN->text()、
          mainwindow.cpp:4657-4663)、PTT が上がる
          (transceiver_ptt(true)、mainwindow.cpp:4711)。条件は
          fTR < 0.75 だけで、遅れた開始も許される。
 +txDelay リグが PTT を確認 → ptt1Timer が
          Configuration::txDelay()（既定 0.200 s、
          Configuration.cpp:1602。FT4 は固定 20 ms、
          mainwindow.cpp:8252-8253）待ってから startTx2()
          (mainwindow.cpp:861-862)。
 0.500 s  送信音声開始。Modulator::start が無音を詰めて波形をちょうど
          ここに置く: delay_ms は FT8 で 500、FT4 で 300、その他 1000
          (Modulator/Modulator.cpp:71-74)。開始が遅れた場合は**ずらさず
          切り詰める** — m_ic = (mstr - delay_ms) * frameRate / 1000
          (Modulator.cpp:94-97) — グリッドは手放さない。
          デコーダ側の基準も一致: xdt = xdt - 0.5
          (lib/ft8_decode.f90:210)。
13.140 s  送信音声終了: 79 × 1920 / 12000 = 12.64 s。
13.640 s  m_bTxTime が閉じる。tx_duration("FT8") = 1.0 + 12.64
          (helper_functions.cpp:7) — 音声終端の 1 s 後までのガードで
          あって、送信が伸びるわけではない。
15.000 s  周期終了。
```

ここから2つ帰結する。どちらも逆に取りやすい。

**0.5 s はトランシーバのものであって、デコーダのものではない。** WSJT-X は
この 0.5 s を PTT 立ち上げ・リグの切替・モジュレータの無音詰めに使う。
メッセージは境界の時点で既に確定している。`txDelay` はこの 0.5 s に
**足される**リードではなく、**内側**で消費される。

**したがって 0.5 s の内側で終わったデコードは、その周期の応答には間に合わ
ない。** `FT8_KEY_UP_AFTER_SLOT_END_US` が stage 3 をそこまで走らせるとして
も、である。締切は2つあり、このパイプラインは片方しか表現していない:
stage 3 の候補取得は slot end + 0.5 s で止まるが、**何を送るか決める**方は
slot end までに終わっていなければならない。実電波（2026-09-19、118スロット、
`logs/udp_ts_2026-09-19.log`）では `post_slotend` が中央値 332 ms /
p90 479 ms — 表現されている締切には常に収まり、応答の締切は大半のスロットで
超えていた。今は送信しないので問題が出ていないだけで、送信が入ったら
見るべき数字は `post_slotend` 対 **0** であって、対 500 ではない。

**この結論は「メッセージは境界で確定する」ことを前提にしている。** それは
WSJT-X の GUI の作りであって、電波側の要件ではない。メッセージを最初に
必要とするのは波形であって PTT ではなく、しかもこの基板は USB 音声の VOX で
リグを送信させるので PTT という手順自体がない。FT4 の節でこれを詰めた。
FT8 については**まだ再評価していない** — 再評価すれば応答の締切は slot end
から音声開始（slot end + 0.5 s）へ移り、つまり
`FT8_KEY_UP_AFTER_SLOT_END_US` が既に表現している位置に重なる。

それでも 0.5 s を残すのは正しい。stage 3 の late パスはフルスロットを必要と
し、フルスロットは境界まで存在しないからである。0.5 s があるから late パスが
成立している — これは送信側からの借金であり、送信するビルドは返す必要がある。

WSJT-X 自身はこの選択を迫られない。`jt9` が別プロセスで、
`MainWindow::decode()` は前のデコードが走っている間、新しいデコードを
**開始しない**だけだからである。この基板は送信機を駆動することになるコアで
デコードしており、本家が考慮しなくてよい条件下にある。

### FT4: 応答は音声開始までに確定していればよい

2026-09-21 に決着。FT4 の 7.5 s 周期では上のどの時刻が効くのかを曖昧に
できない。以前ここでは本家を「内部矛盾」と書いたが、矛盾ではなかった。

**本家が固定している値**（すべてソース実読）:

| 箇所 | 値 | 意味 |
|---|---|---|
| `Modulator/Modulator.cpp:74` | `delay_ms = 300` | 波形は周期開始 0.300 s に PC を出る |
| `lib/ft4/ft4sim.f90:85` | `k = nint((xdt + 0.5)/dt) - NSPS` | **DT = 0 は最初の有効シンボルが 0.500 s に来ること**。105 シンボルの波形はその手前に 48 ms のランプを 1 シンボル持つ |
| `lib/ft4_decode.f90:462` | `xdt = ibest/666.67 - 0.5` | デコーダも同じ 0.5 s 基準 |
| `widgets/mainwindow.cpp:8252` | `if(m_mode=="FT4") ms_delay=20` | FT4 はリレーのシーケンサを待たない |
| `widgets/mainwindow.cpp:1819` | `samples = 21*3456` | WSJT-X の受信側は周期の 6.048 s 分をデコードする |

最初の有効シンボルは 0.348 s に PC を出るので、遅延ゼロの経路なら
**DT −0.15 s** でデコードされる。WSJT-X は FT4 を約 150 ms 早く出している。
素直に読めば送信系（サウンドカード、リグ、PTT／VOX）の遅延分で、FT4 は
`txDelay` を 20 ms に固定しているのでそれを吸収する場所が他にない。ただし
本家にそう書いたコメントはなく、この文書もあるふりはしない。いずれにせよ
0.3 と 0.5 は矛盾ではない。前者は音声が出る時刻、後者はフレームが着く
べき位置である。

**メッセージは波形が始まる時点で存在していればよく、それより前である
必要はない。** WSJT-X が境界で確定させるのは `guiUpdate` が PTT を上げるのと
同じパスでメッセージを読むからで（`mainwindow.cpp:4657-4711`）、PTT は中身を
必要とせず、波形は必要とする。この基板は IC-705 を **USB 音声の VOX** で
送信させるので別途の PTT は存在せず、**音声が出た瞬間に送信がアクティブに
なる**。

`ft4_rx::CAPTURE_CLOSE_SAMPLES`（相手が送信する周期の 6.775 s）から測って:

```text
 周期 N — 相手が送信、この基板は受信
 0.000 s  相手の境界。相手のメッセージ確定と PTT
 0.300 s  相手の音声開始（最初の有効シンボル 0.348 s）
 5.340 s  相手の音声終了（105 × 576 / 12 000 = 5.04 s）
 6.775 s  捕捉終了（0 ms）             -> 候補ループ開始

 周期 N+1 — この基板が送信
 7.500 s  境界（725 ms）               -> 何もしない: VOX なので PTT なし
 7.8 - e  応答確定（約 1 025 ms）      -> 符号化と最初の USB バッファ
 7.800 s  USB 音声開始（1 025 ms）     -> IC-705 が VOX で送信
 7.848 s  最初の有効シンボルが基板を出る
~7.95 s   VOX＋リグ遅延の後に電波が出る -> 相手側で DT ≈ 0
12.840 s  音声終了
```

帰結は3つ:

- **応答の締切は捕捉終了から約 1 025 ms。** 境界の 725 でも、2026-09-21 まで
  `ft4_rx::TX_TURNAROUND_BUDGET_MS` が持っていた 1 225 でもない。後者だと
  音声は 8.0 s に出てフレームは約 200 ms 遅れる — 相手側で DT ≈ +0.2、±1.0 s
  の探索窓の内側ではあるが、相手が自分の時計誤差に必要な余裕を削る。
  捕捉中のベースバンド生成と粗探索で golden の 11 局すべてが CoreS3 上で
  この締切に収まったので、カットも現在はこの締切である。
- **間に合わなかったデコードも捨てない。** その応答には遅いが、画面と
  次々の選択には使う。WSJT-X は送信のためにデコードを止めず、この基板に
  カットがあるのは送信機が必要とするコアでデコードしているからにすぎない。
- **音声をどこまで遅らせてよいかは相手側が決める。** WSJT-X の受信側は周期の
  6.048 s を見るので、5.04 s のフレームが丸ごと入るには相手の境界から約
  1.0 s 以内に始まっている必要がある。

**未解決**: 約 150 ms は WSJT-X が PC のサウンドカードを想定した余裕である。
CoreS3 の UAC 出力経路と IC-705 の VOX 立ち上がりの実遅延は測っておらず、
DT = 0 の「近く」ではなく「上」に着地させるには、この基板の送信を別の
受信機でデコードして DT を読む必要がある。

### 粗同期の探索窓は従属変数である

`stage1_inc` はペア 87 でスペクトログラムを emit し、行 0..173 が埋ま
る。ブロック 2 の最後の Costas シンボルは行 162 にあるので、lag 11 行
までは実データの中に収まり、12 行では収まらない。**0.88 s がこの emit
点の被覆する最大 lag** であり、これが `stage1_inc::max_lag_s` である。

これより広く探索しても、派手には壊れない。埋まっていない行は存在して
値が 0 であり、どちらの総和にも寄与しない。スコアは自己正規化された比
なので、**天井を超えた lag は少ないシンボル数で採点され、全シンボルで
採点されたものと対等に競合する**。

出荷値は `1.0` だったが、`jz = round(lag / 0.08)` により 13 行 =
±1.04 s になっていた。行グリッドが許す全幅をホストミラーで掃引した
結果（fixed-point、録音あたり 51 位相、`mirror_partial_block2_policies`）:

| 窓 | lag 段数 | `qso3_busy` | `qso1` | `qso2` |
|---|---|---|---|---|
| ±0.80 | 21 | 331 | 88 | 87 |
| **±0.88** | **23** | **337** | **118** | **115** |
| ±0.96 | 25 | 335 | 116 | 112 |
...
3 録音すべてで極大、局の消失なし、粗同期の計算量は 15 % 減。実機では
30 スロットで 9.97 ± 1.80、ベースライン 63 スロットの 8.54 ± 1.93 に
対する差が出たが、これはバンドが開いていく時間帯の前後比較である。
ブロック交替ハーネス (`MFSK_FT8_LAG_AB`) で 24 スロット測ると窓自身の
寄与は +0.50 で、ホスト掃引と整合する。統計ではない方の結果として、
deferred 候補がスロットあたり 1.27 から 0 になった。フルスロットを必
要としていた候補が、旧窓の両端にいた候補そのものだったからである。

`decode_pipeline` は窓を `SPEC_EMIT_MAX_LAG_S` にクランプする（一致を
信じるのではなく）。emit 点を動かせば天井も動く。
...
- **キャプチャのスロット内オフセットを加算する。** キャプチャは取得が
  ...
  グリッドが 1.1 s 足りず（per-slot 探索の外。当時は ±1.0 s、現在は ±0.88 s）、2 回目の取得が
...
2026-09-20 まで受信専用で、同日に最初のフレームが無線機へ届いた（デ
ジタル無音として）。この実験で確定したのは、この tree のどこにも書か
れていなかった制約である。

**音声デバイスは無線機ではない。** IC-705 は内蔵 TI ハブの後ろに3デ
バイスとして列挙される。ハブ自身 (`0451:2046`)、CI-V のシリアル対を
2系統持つ Icom の CDC 複合デバイス (`0c26:0036`)、そして **PCM2901
オーディオコーデック** (`08bb:2901`)。音声 OUT はこのコーデックの
インタフェース 1、音声 IN はインタフェース 2 である。

**開けるのは 48 kHz モノラル 16 bit だけ**で、制限は無線機側ではなく
ホストコントローラ側にある。無線機が申告しているにもかかわらず、
`uac_host_device_start` は 2ch / 16bit / 48 kHz を
`ESP_ERR_NOT_SUPPORTED` で拒否する。拒否しているのは `hcd_pipe_alloc`:

```text
E HCD DWC: EP MPS (192) exceeds supported limit (128)
```

ESP32-S3 に HS PHY は無いので `otg_dfifo_depth` は 256 行。既定の
`CONFIG_USB_HOST_HW_BUFFER_BIAS_BALANCED` では
`ptx_fifo_lines = 256/8 = 32` となり、periodic-OUT の MPS 上限は
`32 * 4` = **128 バイト**になる。無線機側の OUT alt 設定:

| alt | フォーマット | maxpkt | レート |
|---|---|---|---|
| 1 | 2ch 16bit | 192 — 上限超過 | 32k / 44.1k / 48k |
| **2** | **1ch 16bit** | **96** | 32k / 44.1k / 48k |
| 3 | 2ch 8bit | 96 | 同上 |
| 4 | 1ch 8bit | 48 | 同上 |

16 bit を保てるのは alt 2 だけ。FT8 の送信は元々モノなので失うものは
無いが、**ストリームもモノとして書かねばならない**。ドライバは受理し
たチャネル数から等時パケット長を決めるので、L = R を流し込むと無線機
がペーシングしている量の2倍を渡すことになる。

**`CONFIG_USB_HOST_HW_BUFFER_BIAS_PERIODIC_OUT` に手を伸ばさないこと。**
上限は ~824 B に上がり alt 1 が通るようになるが、RX FIFO が 160 行か
ら 34 行に落ちる。等時 URB 予算で音声の 2.6〜6.5 % を失った経緯のあ
る基板である（`embedded-poc/CLAUDE.md` 参照）。受信機において、送信
フォーマットのために受信 FIFO を売るのは筋が悪い。

実測、2回とも同一 (`MFSK_CORES3_TX_PROBE=1`、振幅 0): 20 ms のチャン
ク 632 個を **12 603 ms** で送出、音声長 12 640 ms に対し 0.3 % 短い。
つまりこの基板の合成能力ではなく、リング自身のバックプレッシャで律速
されている。GMFSK 合成は `engine::dsp::gfsk::GfskStream`（`sinf` では
なく回転位相子）で、20 ms チャンクあたり約 440 us。

**これはプローブであって送信経路ではない。** 列挙コールバックから走
り、フレームの 12.6 秒間そのタスクを占有する。`RxConnected` はその後
ろで待つので、毎回の捕捉で1スロットを失う。実際の送信機はスロット
グリッド上で QSO ステートマシンから駆動される必要がある。フェーズ計
画は `m5stack-cores3-app/CLAUDE.md` の "TX/QSO feasibility" を参照。
振幅を 0 以外にして走らせる前に、無線機の `PTT SOURCE` が `VOX` でな
いことを確認すること。
