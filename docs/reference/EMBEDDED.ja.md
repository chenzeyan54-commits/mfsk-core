...
FT4 も構造は同じで `delay_ms = 300`、`txDelay = 20 ms`。ただし本家がここで
内部矛盾している — デコーダ側は依然 0.5 s を基準にする
(`xdt = ibest/666.67 - 0.5`、`lib/ft4_decode.f90:462`)。モジュレータ側の
定数が支配するとした場合のコストと、一読だけで変更しなかった理由は
`ft4_rx::TX_TURNAROUND_BUDGET_MS` に記録してある。
