...
* **`SniperRequest<P>`** — narrow-band、単一ターゲット探索。
  `SupportsSniper` でゲートされ、**実装は FT8 のみ**。
  **使う前に下の「sniper モードは roofing filter のためのモード」を
  読むこと** — 「既知の1局を狙う便利機能」ではないし、AP はその一部
  ではない。
  `DecodeRequest::<P>::sniper(audio, target_freq_hz, max_cand)` または
  `SniperRequest::<P>::new(...)` を直接呼ぶ。既定の ±250 Hz 窓を
  変える `.search_hz(w)`、`.osd(bool)`、`.strictness(...)`、
  `.eq_mode(...)`、`.ap_hint(...)`
...
`SniperRequest` について繰り返し誤読される点が3つある。このリポジトリで
...
**2. FT8 専用であり、それが設計である。** `SupportsSniper` を実装して
いるのは `Ft8` だけである。このクレートではどのモードもワイドバンド
経路が主であり、**sniper 無しで WSJT-X 忠実でないなら、それは
ワイドバンド側のバグ**であって、sniper に逃げる理由ではない。FT4 は
帯域全体を回すことが前提のコンテスト用モードなので roofing filter は
目的に反する。FST4 は自前の DDC チャネライザ
(`docs/notes/FST4_DDC_DESIGN.md`) で絞れるので、デコードエンジンを
二重に持つ必要がない。FT4/FST4 の sniper エントリポイントは
2026-09-13 に削除した。C ABI は該当呼び出しに
`MFSK_STATUS_UNKNOWN_PROTOCOL` を返す。

**3. A priori は汎用オプションで、sniper への結合は事故だった（解消
済み）。** AP は確度の高いビットを固定して閾値を 1-3 dB 下げる技術で、
狭帯域である必要はどこにもない。しかし AP は共通エンジン
（`msg::pipeline_ap::decode_sniper_ap`、呼び出し側は sniper だけ）に
同居しており、そのエンジンは候補ループを `if has_ap` で抜けていた —
**hint が付いていることが単一ターゲット化の条件**になっていて、探索幅
は見ていなかった。FT8 が免れていたのは独自の AP 経路を持っていて
このエンジンに入らないからで、`SupportsWideBandAp` が FT8 限定で
あることの中身はそれだけだった。

結合を外すだけでは足りなかった。「一行で解ける」とここで2度断言して
2度とも外したので、計測を残す。ワイドバンドの FT4 デコードをあの
エンジン経由にすると、平文経路が 11 件返すところを 4 件しか返さず、
ヒントを与えた局自身を落とす。しかも hint が「実在する局」でも
「存在しない局」でも**結果が完全に同一** — AP は何も変えておらず、
損失は全部ラダーの違いから来ていた: `process_candidate_ap` は OSD を
depth-2 までしか持たず、depth-3/4 へのエスカレーションも Top-K 救済も
無い。この録音のデコードの多くはまさにそこから来ている。

**そのエンジンは削除した。** AP は
`engine::pipeline::process_candidate_basic` 自身のラダーの最終段に
なった — 上の段はすべて走って失敗した後なので、追加しかできない — 
したがって FT8・FT4・FST4 全サブモードに届く。`msg::pipeline_ap` に
残っているのは `ap_passes`（仮説集合、WSJT-X の `iaptype` 相当）と
`ap_bits_for` の 96 行だけで、エンジンは持たない。

配線の過程で見つかり、いずれも出荷した欠陥が2件ある:

* **FT4/FST4 の AP は、固定ビットの約半分を真値の反対に固定していた。**
  この2つは CRC/FEC の前に 77 ビットメッセージを自身の RVEC と XOR する
  (`ModulationParams::INFO_SCRAMBLE_RVEC`) ので、符号語の中の情報ビットは
  *スクランブル後*のメッセージである。一方 `ApHint` が表すのは
  メッセージそのもの。`ap_bits_for` でスクランブルするようにした。
  −17/−18/−19 dB・各12試行で 3→12、0→11、0→8。FT8 に RVEC は無く
  影響を受けていなかった — AP を最も使うプロトコルで動いていたことが、
  これが生き延びた理由でもある (`tests/ft4_ap_scramble.rs`)。
* **blind CQ 仮説が欠けていた** (本家 `iaptype 1`。局の知識ゼロで
  メッセージ型と `CQ` 接頭辞だけを固定する)。局の知識が無くても効く
  パスなので、ヒント有りの場合ではなく**ベースライン**が動く。FT4 の
  AWGN 閾値が −16.89 → **−18.00 dB**、WSJT-X 公表値 −17.5 dB に対して
  0.6 dB 負けから 0.5 dB 勝ちになった。ただし
  `docs/notes/FT4_BENCHMARK.md` §48 に留保を記録してある:
  スイープコーパスは CQ のみなので、このパスにとって最良条件である。

AP のリスクは捏造デコードなので、ワイドバンド AP は利得アサーションと
同じファイルに精度ガードを置いて出荷した: 送信していない局を指す hint が
その局を生んではならない、純雑音に確信的な hint を与えて何もデコード
されてはならない。
