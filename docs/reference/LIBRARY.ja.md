...
この早期終了は修正済み（エンジンは `decode_band_ap` になり、sniper は
その呼び出し側の一つになった）。同じ作業でもう一つ欠陥が見つかっている:
AP エンジンは FT4 の候補を**汎用**の 2-D Costas 探索で生成していて、
FT4 自身の `getcandidates4.f90` 移植を通っていなかった。候補集合も
`sync_min` の意味（FT4 は baseline 正規化、汎用探索はそうでない）も
両方とも間違っていたことになる。

**どちらを直してもワイドバンド AP は成立しない。そしてその理由こそ
記録する価値がある。** WSJT-X の FT4 golden で実測: ワイドバンドの
デコードを AP エンジン経由にすると、平文経路が 11 件返すところを 4 件
しか返さず、ヒントを与えた局自身を落とす。しかも hint が「実在する局」
でも「存在しない局」でも**結果が完全に同一** — つまり AP は何も変えて
おらず、損失は全部「別のラダーを通っていること」から来ている。捏造は
ゼロなので、これは偽デコードではなく recall の問題である:
`process_candidate_ap` は OSD を depth-2 までしか持たず、depth-3/4 への
エスカレーションも Top-K 救済も無い。この録音のデコードの多くは
まさにそこから来ている。

したがって `SupportsWideBandAp` は「FT4/FST4 はワイドバンド AP が
できない」という意味でもなければ、一行で解ける話でもない。**AP が
本来のラダーとは別の、浅いラダーに同居している**という意味である。
ワイドバンド AP とは、ワイドバンドエンジンが実際に使うラダー
(`process_candidate_basic`) に AP オプションを与えることであり、
その上でなお偽デコードの計測が要る。AP のリスクは捏造だからである。
