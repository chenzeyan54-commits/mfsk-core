...
M5Stack CoreS3 上で、リアルタイムに FT8 が動きます。
...
アルゴリズムはそのまま。<span style="color:var(--accent)">動く場所を増やす。</span>
...
数字はテストが assert しており、下回ればテストで落ちます。
...
純 Rust。FFT も外部から差し替え可能。 `no_std + alloc` でビルドでき、C からも呼べる ABI を用意しています。

### F03-15 · まとめ帯

参照実装を置き換えるのではなく、対応できないユースケースに対応するためのライブラリです。
...
再現率だけではベンチマークにはなりません。 デコーダは原理上存在しないコールサインを出力します。正常なデコーダと区別するために適合率(precision) も常にベンチマークしています。
...
速さと正しさをバランスよく。取り逃さないことと、でっち上げないことを、同時に測っています。
...
AMD Ryzen 9 9900X (12C/24T) / `--release --features full`。 WSPR が他より遅いのは意図的です。`wsprd` の DT ピーク探索を入れて約 1 dB の感度と引き換えにしています。
...
**FT8 は2つの構成があります。** ホスト構成（`DecodeDepth::FULL`）は研究用の全力設定。組み込み構成（`EMBEDDED`）は ESP32 の時間予算に合わせて意図的に軽くした別のデコーダで、劣化版ではありません。 FT8 の QSO 折り返し予算はスロット終了後 **約2秒** — その中で復調・ウォーターフォール描画・次スロットの送信準備まで済ませる必要があります。
...
- **FFT の外部実装** — `fft-extern` でファクトリ関数を1つ提供するだけ。ESP32 なら esp-dsp をそのまま使えます。
...
解決は Goertzel 再帰への置き換え。==スクラッチ 0、速度は据え置き、 精度はむしろ +0.16〜+0.63 dB 改善==。
...
本ライブラリは WSJT-X（K1JT Joe Taylor 氏および共同開発者の皆さん）の Rust 再実装です。 すべてのアルゴリズムの出典は WSJT-X にあり、参照実装はこれからも WSJT-X です。

### F10-11 · まとめ帯

WSJT-X のデコーダを Rust で、オープンに、どのような環境でも動かせることを目指しています。
...
**WSJT-X はこれからも参照実装です。**各ファイルは移植元の `lib/*.f90` / `lib/*.c` を明記しています。 このライブラリの存在理由は WSJT-X を置き換えることではなく、 ==Fortran + C + Qt のデスクトップアプリが対応できない環境で、同じデコーダを動かすこと==です。
...
### L01-30 · 本文
WSJT-X 配布の実録音に対する golden テストと、AWGN 掃引による感度測定。 **再現率（recall）だけでなく誤デコード（precision）も**常に測ります。
数字はテストが assert しており、下回ればリリース前テストで落ちます。FST4・Q65 を含む全モードは `docs/notes/BENCHMARKS.md` に。
...
crates.io / docs.rs — `mfsk-core`（最新リリース v0.10.0、0.10.1 準備中）／ GPL-3.0-or-later 
参考資料：`README.md`／`LIBRARY.md`（ホスト API）／`EMBEDDED.md`（組み込み・C ABI）／`BENCHMARKS.md`（本紙の数字の出どころ）。`docs/reference/` は全文書に日本語版があります。

### L02-29 · フッタ

<span>本ライブラリは WSJT-X（K1JT Joe Taylor 氏ほか）の Rust 再実装です。すべてのアルゴリズムの出典は WSJT-X にあり、参照実装はこれからも WSJT-X です。</span>
